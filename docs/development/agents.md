...
> กฎบังคับทุก session: `.cursor/rules/bagisto-customization.mdc`, `.cursor/rules/bagisto-workflow.mdc`, `.cursor/rules/bagisto-performance.mdc`, `.cursor/rules/bagisto-security.mdc`  
> สิ่งที่แก้นอก upstream: [customizations.md](customizations.md)

## Do Not Edit

- `vendor/`, `node_modules/`, `composer.lock`, `package-lock.json`
- `packages/Webkul/*` — **ห้ามแก้โดยตรง** (ยกเว้นบันทึกใน [customizations.md](customizations.md) และมีแผน patch/custom package) — ดู `.cursor/rules/bagisto-customization.mdc`
