<x-admin::layouts>
    ...
    <div class="flex items-center justify-between gap-4 mb-5 max-sm:flex-wrap">
        <div class="grid gap-1.5">
            <p class="font-display text-2xl font-semibold !leading-normal text-admin-text" v-pre>
                @lang('admin::app.dashboard.index.user-name', ['user_name' => auth()->guard('admin')->user()->name])
            </p>

            <p class="!leading-normal text-admin-muted">
    ...
    <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
        ...
        <div class="flex w-[360px] max-w-full flex-col gap-2 max-sm:w-full">
            ...
            <!-- Store Stats -->
            <div class="box-shadow rounded-lg bg-admin-card ">
                <!-- Total Sales Details -->
        ...
        >
            <div class="flex gap-1.5">
                <template v-if="channels.length > 2">
                    <x-admin::dropdown position="bottom-right">
                        <x-slot:toggle>
                            <button
                                type="button"
                                class="inline-flex w-full cursor-pointer appearance-none items-center justify-between gap-x-2 rounded-md border border-admin-border bg-admin-card px-2.5 py-1.5 text-center text-sm leading-6 text-gray-600 transition-all marker:shadow hover:border-gray-400 focus:border-gray-400 border-admin-border  text-admin-muted hover:border-admin-primary/40 focus:border-admin-primary"
                            >
                        ...
                        <x-slot:menu class="!p-0 shadow-[0_5px_20px_rgba(0,0,0,0.15)] border-admin-border">
                            <x-admin::dropdown.menu.item
                                v-for="channel in channels"
                                ::class="{'bg-admin-surface': channel.code == filters.channel}"
                ...
                <x-admin::flat-picker.date class="!w-[140px]" ::allow-input="false">
                    <input
                        class="flex min-h-[39px] w-full rounded-md border px-3 py-2 text-sm text-admin-muted transition-all hover:border-admin-primary/40 border-admin-border bg-admin-card  text-admin-muted hover:border-admin-primary/40"
