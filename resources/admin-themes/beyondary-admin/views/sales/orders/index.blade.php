<x-admin::layouts>
    ...
    <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
        <p class="py-3 font-display text-2xl font-semibold text-admin-text">
            @lang('admin::app.sales.orders.index.title')
        </p>
    ...
    <x-admin::datagrid :src="route('admin.sales.orders.index')" :isMultiRow="true">
        <template #header="{
            ...
            <template v-else>
                <!-- Grid Header Columns -->
                <div class="row grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 items-center border-b px-2 sm:px-4 py-2.5 border-admin-border">
                    <div
                    ...
                    >
                        <p class="text-admin-muted text-sm sm:text-base">
        ...
        <template #body="{
            ...
            <template v-else>
                <!-- Order Rows -->
                <div
                    class="row grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-y-4 border-b px-2 sm:px-4 py-2.5 transition-all hover:bg-admin-surface border-admin-border"
                    v-for="record in available.records"
                >
                    <!-- Order Id, Created, Status Section -->
                    <div class="flex flex-col gap-1.5">
                        <p class="text-sm sm:text-base font-semibold text-admin-text">
                            @{{ "@lang('admin::app.sales.orders.index.datagrid.id')".replace(':id', record.increment_id) }}
                        </p>

                        <p class="text-xs sm:text-sm text-admin-muted">
                            @{{ record.created_at }}
                        </p>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <p class="text-sm sm:text-base text-admin-text">
                            @{{ record.full_name }}
                        </p>
                    ...
                    <div class="flex items-center justify-between gap-x-2">
                        ...
                        <a :href="'{{ route('admin.sales.orders.view', ':id') }}'.replace(':id', record.id)">
                            <span class="icon-sort-right rtl:icon-sort-left cursor-pointer p-1.5 text-xl sm:text-2xl hover:rounded-md hover:bg-admin-surface ltr:ml-1 rtl:mr-1"></span>
                        </a>
                    </div>
                </div>
            </template>
                ...
                >
                    ...
                    <x-slot:header>
                        <div class="grid gap-3">
                            <p class="py-2 font-display text-xl font-medium text-admin-text">
                            ...
                            <div class="relative w-full">
                                <input
                                    type="text"
                                    class="block w-full rounded-lg border bg-white py-1.5 leading-6 text-gray-600 transition-all hover:border-gray-400 border-admin-border  text-admin-muted ltr:pl-3 ltr:pr-10 rtl:pl-10 rtl:pr-3"
                    ...
                    <x-slot:content class="!p-0">
                        ...
                        >
                            <div
                                class="grid cursor-pointer place-content-start gap-1.5 border-b border-slate-300 p-4 last:border-b-0 hover:bg-admin-surface border-admin-border"
                                v-for="customer in searchedCustomers"
                                @click="createCart(customer)"
                            >
                                <p class="text-base font-semibold text-admin-muted">
